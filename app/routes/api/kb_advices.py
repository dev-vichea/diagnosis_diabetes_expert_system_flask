...
def get_advice(advice_id: int):
    ...
    return {
        ...
        "title": a.title,
        "content": a.content,
        "recommendations": a.recommendations_json,
        "severity": a.severity,
        "is_active": bool(a.is_active),
    }
...
def create_advice():
    ...
    if recommendations is None and "recommendations_json" in data:
        recommendations = data.get("recommendations_json")
    if isinstance(recommendations, str) and recommendations.strip():
        try:
            recommendations = json.loads(recommendations)
        except json.JSONDecodeError:
            return {"message": "recommendations must be valid JSON"}, 400
...
def update_advice(advice_id: int):
    ...
    if "recommendations" in data or "recommendations_json" in data:
        recommendations = data.get("recommendations")
        if recommendations is None and "recommendations_json" in data:
            recommendations = data.get("recommendations_json")
        if isinstance(recommendations, str) and recommendations.strip():
            try:
                ...
                return {"message": "recommendations must be valid JSON"}, 400
        a.recommendations_json = recommendations
